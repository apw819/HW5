...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/alliwanHW5"
...
app.config['MAIL_SENDER'] = 'Admin <{}>'.format(os.environ.get('ADMIN')) # TODO fill in email
...
def index():
    ...
    if form.validate_on_submit():
        if db.session.query(Tweet).filter_by(text=form.text.data, user_id= (get_or_create_user(db.session, form.username.data, form.email.data).id)).first():
            ...
            sendemailtowhensubmit(app.config['ADMIN'], 'New Tweet Added', 'mail/new_tweet', tweet_text=form.text.data)
